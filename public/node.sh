#!/bin/bash
# Node Installer Script for Nuvyra Panel
...
echo "======================================"
echo "    Nuvyra Panel Node Setup Script       "
echo "======================================"
...
mkdir -p /opt/nuvyra-panel-node
cd /opt/nuvyra-panel-node

# Create package.json
cat << 'PKGEOF' > package.json
{
  "name": "nuvyra-panel-node",
  "version": "1.0.0",
  "description": "Node agent for Nuvyra Panel",
  "main": "agent.js",
  "dependencies": {
    "express": "^4.18.2",
...
pm2 stop nuvyra-node 2>/dev/null || true
pm2 delete nuvyra-node 2>/dev/null || true
pm2 start agent.js --name nuvyra-node
pm2 save
